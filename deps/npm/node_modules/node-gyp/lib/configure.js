...
async function configure (gyp, argv) {
  ...
  async function getNodeDir () {
    ...
    if (!gyp.opts.nodedir &&
        (process.config.variables.use_prefix_to_find_headers || process.platform === 'openharmony')) {
      // OpenHarmony distributions are relocatable. Find their bundled headers
      // next to the running executable; other platforms use the configured
      // prefix. Only use local headers when they match the requested version.
      const prefix = process.platform === 'openharmony'
